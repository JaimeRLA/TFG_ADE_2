...
**Distribución de Riqueza:** El capital inicial de cada clúster (`self.saldo_inicial`) no es uniforme; refleja la heterogeneidad del sistema bancario real. El cálculo se realiza en dos etapas:

1. **Asignación de Base:** Se sortea un valor aleatorio uniforme (`U`) dentro de los rangos `SALDO_RETAIL_RANGO` para clientes minoristas, `SALDO_VIP_RANGO` para patrimonios VIP y `SALDO_EMPRESA_RANGO` para cuentas corporativas.

2. **Escalado por Perfil:** El valor base se multiplica por un factor de ponderación específico al segmento del agente (`M_tipo`), donde los multiplicadores se seleccionan según el clúster: `MULTIPLICADOR_RETAIL`, `MULTIPLICADOR_VIP` o `MULTIPLICADOR_EMPRESA`.

Matemáticamente: **Sᵢ = U(SALDO_RANGO_tipo) · MULTIPLICADOR_tipo**

Finalmente, todos los saldos se normalizan proporcionalmente mediante un factor de escala para que su suma coincida exactamente con `total_depositos` configurado en el preset, garantizando la consistencia del balance bancario desde el turno 0.
